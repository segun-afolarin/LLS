...
/*
  FRONTEND-ONLY NOTE
  -------------------
  LLS has no backend connected yet. Everything below runs on local
  mock state instead of api calls. When the backend is ready, swap:
    - MOCK_NOTIFICATIONS        -> GET /notifications
    - handleMarkAllRead body    -> PATCH /notifications/read-all
    - handleMarkRead body       -> PATCH /notifications/:id/read
    - handleDismiss body        -> DELETE /notifications/:id
  The component shape, state names, and UI won't need to change —
  only the bodies of those three handlers and the initial fetch.
*/
...
const MOCK_NOTIFICATIONS = [
  {
    id: 1,
    type: "alert",
    title: "New harassment report filed",
    message: "A student on Abuja campus submitted a harassment report requiring urgent review.",
    time: "2m ago",
    read: false,
  },
  {
    ...
    message: "\"Water shortage — Block C hostel\" has reached community confirmation threshold.",
    time: "18m ago",
    read: false,
  },
  {
  ...
  },
  {
    ...
    message: "Your campus activity summary for this week is available.",
    time: "Yesterday",
    read: true,
  },
];
        ...
        <div className="flex items-center">
          {["all", "unread"].map((tab) => (
            <button key={tab} onClick={() => setFilter(tab)}
              ...
              {filter === tab && <motion.div layoutId="notif-tab" className="absolute bottom-0 left-0 right-0 h-[2px] bg-primary" transition={{ duration: 0.2, ease: [0.16, 1, 0.3, 1] }} />}
            </button>
          ))}
        </div>
      </div>
...
const DashboardHeader = ({ darkMode, setDarkMode, sidebarOpen, setSidebarOpen, mobileSidebar, setMobileSidebar }) => {
  ...
  const initials  = parts.length >= 2
    ...
    : parts[0] ? parts[0].slice(0, 2).toUpperCase() : "ST";
